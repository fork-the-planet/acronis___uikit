...
3. **Theming mirrors legacy, on the new pipeline.** Same runtime model as
   ...
   `design-tokens` → `design-theme` (Style Dictionary)** and pairs with the new
...
| Theme build      | `design-theme` + `tools/style-dictionary` | 0.5.0   | Multi-brand emerging                   |
...
- [ ] **Token contract & versioning** — define what a breaking token change is; how
      `design-tokens` semver maps to `design-theme` and consumers.
